...
export default function Dashboard() {
  ...
  // Fetch demographics (once on mount)
  useEffect(() => {
    // Check localStorage cache
    try {
      const raw = localStorage.getItem(DEMOGRAPHICS_CACHE_KEY);
      if (raw) {
        const { demographics: cached, timestamp } = JSON.parse(raw);
        if (Date.now() - timestamp < DEMOGRAPHICS_CACHE_TTL) {
    ...
    fetch("/api/demographics")
      ...
      .then((data) => {
        if (data?.demographics) {
          ...
          try {
            localStorage.setItem(DEMOGRAPHICS_CACHE_KEY, JSON.stringify({ demographics: data.demographics, timestamp: Date.now() }));
          } catch {}
        }
      })
      .catch(() => {})
...
{!isTikTok && !isSimplePlatform && (
      ...
      {isInstagram && !demographicsLoading && demographics.length > 0 && (() => {
        ...
        return (
          <div className="mt-10">
            <h2 className="text-sm font-medium text-black mb-6">Audience</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
              {/* Gender */}
              {genderTotal > 0 && (
                <div>
                  <h3 className="text-xs font-medium text-neutral-400 uppercase tracking-wider mb-4">Gender</h3>
                  <AudienceGenderChart genderData={genderData} genderTotal={genderTotal} />
                </div>
              )}
              ...
              {ageData.length > 0 && (
                <div>
                  <h3 className="text-xs font-medium text-neutral-400 uppercase tracking-wider mb-4">Age</h3>
            ...
            {countryData.length > 0 && (
              <div>
                <h3 className="text-xs font-medium text-neutral-400 uppercase tracking-wider mb-4">Top Countries</h3>
                    ...
                    .map((item, idx) => (
                      <div key={item.label} className="flex items-center gap-4 py-3 border-b border-neutral-100 last:border-0">
                        ...
                        <div className="w-32 h-1 bg-neutral-100 rounded-full overflow-hidden">
                          <div className="h-full bg-black rounded-full" style={{ width: `${(item.value / maxCountryValue) * 100}%` }} />
                        </div>
                        <span className="text-sm text-neutral-400 w-16 text-right">{countryTotal > 0 ? Math.round((item.value / countryTotal) * 100) : 0}%</span>
                      </div>
                    ))}
                </div>
              </div>
...
const audienceGenderColors: Record<string, string> = {
  M: "#4A5FD9",
  F: "#E05C97",
  U: "#9CA3AF",
};

const audienceGenderConfig: ChartConfig = {
  M: { label: "Male", color: "#4A5FD9" },
  F: { label: "Female", color: "#E05C97" },
  U: { label: "Other", color: "#9CA3AF" },
};

function AudienceGenderChart({ genderData, genderTotal }: { genderData: DemographicItem[]; genderTotal: number }) {
  const chartData = genderData.map((item) => ({
    gender: item.label,
    value: item.value,
    fill: audienceGenderColors[item.label] || "#9CA3AF",
  }));

  return (
    <div>
      <ChartContainer config={audienceGenderConfig} className="mx-auto aspect-square max-h-[180px]">
        <PieChart>
          <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel />} />
          <Pie data={chartData} dataKey="value" nameKey="gender" innerRadius={50} outerRadius={75} strokeWidth={2} stroke="#fff">
            <Label
              content={({ viewBox }) => {
                if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                  return (
                    <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle" dominantBaseline="middle">
                      <tspan x={viewBox.cx} y={viewBox.cy} className="fill-black text-2xl font-bold">
                        {genderTotal.toLocaleString()}
                      </tspan>
                      <tspan x={viewBox.cx} y={(viewBox.cy || 0) + 20} className="fill-neutral-400 text-xs">
                        followers
                      </tspan>
                    </text>
                  );
        ...
        {chartData.map((item) => {
          ...
          return (
            <div key={item.gender} className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: audienceGenderColors[item.gender] || "#9CA3AF" }} />
              <span className="text-xs text-neutral-500">{audienceGenderConfig[item.gender]?.label || item.gender}</span>
